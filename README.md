...
Fresh or explicitly searched questions use a server-only free-tier search cascade in this order: Brave Web Search, Brave Answers, both Tavily keys, Exa, then DuckDuckGo Instant Answers. Failed, empty, unauthorized, or quota-exhausted providers are cooled down and the next provider is tried automatically. Successful searches are cached for five minutes to conserve quota. Tavily is fixed to one-credit `basic` searches, Exa uses bounded `instant` highlights, and Brave Answers does not enable research mode. Set `WEB_SEARCH_CACHE_TTL_MS` to adjust the cache. Keep paid overage disabled in every provider dashboard for a strict zero-cost guarantee; application-side limits cannot override an account that has billing enabled.
...
Authentication is federated through the central Busted Minds Account at `accounts.bustedminds.org`. BMAI never invokes Google directly; it uses the `custom:busted-minds` OIDC provider and keeps its application data in the separate `bmai` Supabase project. Busted Minds Search uses the dedicated `custom:busted-minds-search` client branding while returning to the same BMAI session and user.
